...
# Fachlicher Hinweis zur Flächenermittlung
st.warning("⚠️ **Wichtiger Hinweis:** Gemessen werden nur **unüberdachte Flächen**, auf die der Regen unmittelbar niederfällt. Überdachte Bereiche (z. B. unter Tankstellendächern) werden für $Q_r$ nicht berücksichtigt.")
...
a_hof = flaeche_zeile("Hof- / Freifläche (unüberdacht)", "hof")
a_wasch = flaeche_zeile("Waschplatz (außen / unüberdacht)", "wasch")
a_tank = flaeche_zeile("Tankfläche (nur unüberdachter Teil)", "tank")
a_lager = flaeche_zeile("Lager- / Abstellfläche (unüberdacht)", "lager")

# Schlagregen-Berechnung (Faktor 0,6)
st.markdown("---")
st.subheader("Schlagregen (Dach/Wand)")
with st.expander("ℹ️ Info: Warum Faktor 0,6?"):
    st.write("Schlagregen fällt bei Wind im 30°-Winkel gegen die Senkrechte. Daher werden 60% (Faktor 0,6) der vertikalen Wandfläche als Regeneintrag gewertet.")
...
total_area = a_hof + a_wasch + a_tank + a_lager + a_schlag
...
with st.expander("⚠️ Wichtig: Messung der Ventildimension"):
    ...
    *   **Ventil 1/2\"** (DN 15) -> 3/4\" am Schlauchanschluss.
    *   **Ventil 3/4\"** (DN 20) -> 1\" am Schlauchanschluss.
    *   **Ventil 1\"** (DN 25) -> 1 1/4\" am Schlauchanschluss.
    """)
...
# Erschwernisfaktor fx (Korrektur: a_wasch ist nun definiert)
...
def create_pdf():
    ...
    details = (
        f"Regenspende: {r_spende} l/(s*ha)\n"
        f"Gesamtflaeche: {total_area:.2f} m2 (inkl. Schlagregen)\n"
        f"Schmutzwasser Qs: {qs:.2f} l/s\n"
        f"Dichtefaktor fd: {fd} | FAME-Faktor ff: {ff}\n"
        f"Erforderliches Schlammfangvolumen: {v_final:.2f} Liter"
    )
    pdf.multi_cell(190, 7, txt(details))
