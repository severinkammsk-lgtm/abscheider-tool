...
# --- 5. SCHLAMMFANG (EXAKT NACH DEINER TABELLE) ---
...
if is_wash_plant:
    ...
    anfall = st.radio("Erwarteter Schlammanfall", ["Kein", "Gering", "Mittel", "Groß"], index=1)
    
    # Detail-Infos aus deiner Tabelle für die Bewertung
    if anfall == "Kein":
        st.write("Criteria: Kondensat")
        v_sf = 0.0
    elif anfall == "Gering":
        st.info("Regenauffangflächen mit wenig Schmutz, Auffangtassen, überdachte Tankstellen.")
        v_sf = (100 * ns) / (fd * ff)
    elif anfall == "Mittel":
        st.info("Tankstellen, PKW-Wäsche von Hand, Teilewäsche, Omnibus-Waschstände, Reparaturwerkstätten, Fahrzeugabstellflächen.")
        # Formel: 200 * NS / fd / ff | Mindestvolumen 600l
        v_sf = max((200 * ns) / (fd * ff), 600.0)
    elif anfall == "Groß":
        st.info("Waschplätze für LKW, Baustellenfahrzeuge, Landwirtschaftsfahrzeuge.")
        # Formel: 300 * NS / fd / ff | Mindestvolumen 5000l
        v_sf = max((300 * ns) / (fd * ff), 5000.0)

st.metric("Erforderliches Schlammvolumen", f"{v_sf:.0f} Liter")
