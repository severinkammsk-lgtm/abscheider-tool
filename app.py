...
def schlagregen_zeile(label, key_suffix):
    ...
    res = l * (h * 0.6) # Wind 30 Grad gegen Senkrechte (0.6*DH) 
    with c3:
        st.markdown(f"<div style='padding-top:35px'>= <b>{res:.2f} m²</b></div>", unsafe_allow_html=True)
    return res, l, h
...
total_area = a_tank + a_hof + a_wasch + a_lager + a_schlag # [cite: 10]
qr = (r_spende * total_area) / 10000 # [cite: 11]
...
# Logik: HD-Reiniger 1,0 l/s wenn Waschanlage vorhanden, sonst 2,0 l/s fürs Erstgerät 
qs_hd = 0.0
if anz_hd > 0:
    if is_wash:
        qs_hd = anz_hd * 1.0
...
qs = qs1_total + qs_w + qs_hd # [cite: 23]
...
fx = 2.0 if (a_wasch > 0 or is_wash or anz_hd > 0 or qs1_total > 0) else 1.0 # [cite: 28]
...
fd = fd_map[dichte][at] # [cite: 39, 46]
...
if is_wash:
    ...
    v_sf_calc = (sf_faktor * ns) / fd if fd > 0 else 0 # [cite: 64]
...
logo_path = "logo.png" # Muss im Root liegen
...
def create_pdf():
    ...
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(190, 6, txt(f"Die Regenspende i: {r_spende} l/(s*ha) basiert auf oertlichen Wetterdaten (Regendauer D=5 min, Jaehrlichkeit T=2 Jahre) gemaess DIN 1986-100."))
    pdf.ln(2)
    ...
    # Pos 3: Faktoren Begründung 
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 8, txt(" 3. Faktoren und Anlagentyp (DIN 1999-100)"), ln=True, fill=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(190, 6, txt(f"Erschwernisfaktor fx = {fx}: Erhoeht auf 2,0 bei Schmutzwasserbehandlung, da Reinigungs- oder Hilfsmittel die Schwerkrafttrennung erschweren. [cite: 28]"))
    pdf.multi_cell(190, 6, txt(f"Dichtefaktor fd = {fd}: Kompensiert die Abweichung der Fluessigkeitsdichte ({dichte} g/cm3) gemaess DIN EN 858-2. [cite: 35, 39]"))
    pdf.multi_cell(190, 6, txt(f"FAME-Faktor ff = {ff}: Beruecksichtigt den Biodiesel-Anteil ({fame}), der das Abscheideverhalten maßgeblich beeinflusst. [cite: 48, 49]"))
    pdf.ln(4)

    # Pos 4: NS Begründung 
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 8, txt(f" 4. Ergebnis Nenngröße NS {ns:.2f}"), ln=True, fill=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(190, 6, txt(f"Die Nenngroeße stellt sicher, dass auch bei gleichzeitiger Belastung durch Starkregen und maximalem Schmutzwasserabfluss die erforderliche Verweilzeit fuer eine normgerechte Abscheidung eingehalten wird. Dies verhindert das Mitreißen von Leichtfluessigkeiten in das Kanalnetz. "))
    pdf.ln(4)

    # Pos 5: Schlammvolumen [cite: 63, 64]
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 8, txt(" 5. Schlammfangvolumen (V)"), ln=True, fill=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(190, 6, txt(f"Berechnetes Volumen: {v_final:.2f} Liter (Faktor {sf_faktor}%)."))
    pdf.multi_cell(190, 6, txt(f"Begründung: {bew_t}. "))
    if v_final == v_min:
        pdf.multi_cell(190, 6, txt(f"Hinweis: Das Volumen wurde auf das normative Mindestvolumen von {v_min} l gemaess DIN 1999-100 erhoeht. "))

    # Firmendaten Footer
    pdf.ln(10)
    pdf.set_font("Arial", '', 8)
    pdf.cell(47.5, 5, txt("Gf. Michael Wintergerst"), ln=0)
    pdf.cell(47.5, 5, txt("Tel: 07052-50 89 79 0"), ln=0)
    pdf.cell(47.5, 5, txt("Fax: 07052-50 89 79 9"), ln=1)
    pdf.cell(47.5, 5, txt("Im Gewand 8"), ln=0)
    pdf.cell(95, 5, txt("info@al-wintergerst.de"), ln=0)
    pdf.cell(47.5, 5, txt("75378 Bad Liebenzell"), ln=1)
