...
# Hilfsfunktion zur Ventilberechnung nach Tabelle 1 (Gruppenbasierte Berechnung für Qs1)
...
# Spezialfunktion für Schlagregen
...
if is_wash:
    v_sf_calc = 5000.0
    bew_t = "Fahrzeugwaschanlagen (Mindestwert 5.000 l nach DIN 1999-100)"
    sf_faktor = 0
else:
...
def create_pdf():
    ...
    pdf.ln(2)
    pdf.cell(100, 7, txt(f"Gesamtfläche inkl. Schlagregenanteil:"))
    pdf.cell(90, 7, f"{total_area:.2f} m2", ln=True, align='R')
    pdf.cell(100, 7, txt(f"Maximaler Regenabfluss:"))
    ...
    pdf.set_font("Arial", '', 10)
    pdf.cell(100, 6, txt(f"Summe Auslaufventile DN 15 ({v15_c} Stk.):"))
    pdf.cell(90, 6, f"{qs1_15:.2f} l/s", ln=True, align='R')
    pdf.cell(100, 6, txt(f"Summe Auslaufventile DN 20 ({v20_c} Stk.):"))
    pdf.cell(90, 6, f"{qs1_20:.2f} l/s", ln=True, align='R')
    pdf.cell(100, 6, txt(f"Summe Auslaufventile DN 25 ({v25_c} Stk.):"))
    ...
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(190, 6, txt("Die Schlammfangmenge ergibt sich aus (Faktor * NS) / fd. Es werden die normativen Mindestwerte von 600 l, 2500 l oder 5000 l berücksichtigt."))
    pdf.ln(2)
