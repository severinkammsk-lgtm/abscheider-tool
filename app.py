...
# Immer aufrunden auf 2 Dezimalstellen für die Sicherheit
...
# Erstes Gerät 2,0 l/s, wenn keine Waschanlage da ist. Mit Waschanlage zählt das erste Gerät nur 1,0 l/s.
if anz_hd > 0:
...
fame = st.selectbox("Biodiesel (FAME)", ["bis 5 %", "5 - 10 %", "über 10 %"])
...
if is_wash:
    ...
    # Gering = *100, Mittel = *200, Groß = *300
    ...
    bew_map = {
        ...
        200: "Tankstellen, PKW-Wäsche von Hand, Werkstätten, Fahrzeugabstellflächen, Kraftwerke",
        300: "Waschplätze für LKW, Baumaschinen, landwirtschaftliche Maschinen"
    }
    bew_t = bew_map[sf_faktor]
    st.info(f"**Bewertung:** {bew_t}")

# Mindestvolumina nach DIN 1999-100
v_min = 0.0
if ns > 0:
...
def create_pdf():
    ...
    pdf.set_font("Arial", '', 8)
    pdf.multi_cell(190, 4, txt(f"Regenspende i: {r_spende} l/(s*ha) gemaess DIN 1986-100 (D=5, T=2). Das Ergebnis wurde aufgerundet."))
    pdf.cell(100, 5, txt("Fläche inkl. Schlagregenanteil:"))
    pdf.cell(90, 5, f"{total_area:.2f} m2", ln=True, align='R')
    ...
    pdf.set_font("Arial", '', 8)
    pdf.cell(100, 4, txt(f"Ventile (DN15: {v15_c}, DN20: {v20_c}, DN25: {v25_c} Stk.):"))
    pdf.cell(90, 4, f"{qs1_total:.2f} l/s", ln=True, align='R')
    if qs_w > 0:
        pdf.cell(100, 4, txt(f"Waschanlage ({wasch_typ}):"))
        pdf.cell(90, 4, f"{qs_w:.2f} l/s", ln=True, align='R')
    if qs_hd > 0:
        pdf.cell(100, 4, txt(f"Hochdruckreiniger ({anz_hd} Stk.):"))
        pdf.cell(90, 4, f"{qs_hd:.2f} l/s", ln=True, align='R')
    pdf.set_font("Arial", 'B', 8.5)
    ...
    pdf.set_font("Arial", '', 8)
    pdf.multi_cell(190, 4, txt(f"Anlagentyp: {at} | fx={fx}: Erschwernisfaktor | fd={fd}: Dichtefaktor | ff={ff}: FAME-Faktor. Diese Faktoren korrigieren die hydraulische Kapazitaet gemaess Stoffeigenschaften."))
    pdf.ln(2)

    # Pos 4: NS (Ohne Linie unten)
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(190, 6, txt(" 4. Ergebnis Nenngröße (NS)"), ln=True, fill=True)
    pdf.set_font("Arial", 'B', 10)
    pdf.cell(100, 8, txt(" Erforderliche Nenngröße:"))
    pdf.cell(90, 8, f"NS {ns:.2f}", ln=True, align='R')
    pdf.set_font("Arial", '', 8)
    pdf.multi_cell(190, 4, txt("Die Nenngroeße stellt die Kapazitaet zur Einhaltung der normativen Verweilzeit bei maximaler hydraulischer Belastung sicher."))
    pdf.ln(2)

    # Pos 5: Schlammvolumen (Ohne Linie unten)
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(190, 6, txt(" 5. Schlammfangvolumen (V)"), ln=True, fill=True)
    pdf.set_font("Arial", 'B', 10)
    pdf.cell(100, 8, txt(" Gewähltes Schlammvolumen:"))
    pdf.cell(90, 8, f"{v_final:.2f} Liter", ln=True, align='R')
    pdf.set_font("Arial", '', 8)
    pdf.multi_cell(190, 4, txt(f"Berechnung: (Faktor {sf_faktor}% * NS) / fd. Einstufung: {bew_t}. Das Volumen entspricht den gesetzlichen Mindestvorgaben."))
